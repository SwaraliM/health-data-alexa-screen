...
### Evaluative & generic follow-ups
`resolveAlexaTurn` also short-circuits **evaluative** follow-ups (`looksLikeEvaluativeFollowup`: "is that good or bad?", "is this normal for my age?", "should I be worried?", "is that enough?") to `chart_qna` (action `evaluate`) when a chart is visible — previously these fell to `ignore_chatter`. And generic wellness openers (`looksLikeGenericWellness`: "how am I doing?", "how have I been lately?", "how's my health?") now count as health questions in `isExplicitHealthQuestion`, so they start an overview instead of falling through. The chart-QnA answer prompt now (a) ALWAYS defines metrics from the glossary for definition questions (never "I can't tell from this chart"), and (b) gives gentle good/bad/normal judgements for evaluative questions using typical healthy ranges, deferring to a doctor for concerns without alarming.
